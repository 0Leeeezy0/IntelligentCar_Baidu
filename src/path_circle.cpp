...
CircleTrackStep CircleTrack_Step_Judge(Img_Store *Img_Store_p,Data_Path *Data_Path_p)
{
    ...
    int CircleStepFlag = 0; // 圆环步骤决策标志位
    switch(Data_Path_p -> Track_Kind)
    {
        case L_CIRCLE_TRACK:
        {
            for(int i = -10;i <= 10;i++)
            {
                if((Img_Store_p -> Img_OTSU_Unpivot).at<uchar>((Data_Path_p -> InterruptCoordinate[0][1]-10),(Data_Path_p -> InterruptCoordinate[0][0])+i) == 255)
                {
                    CircleStepFlag = 1;
                    break;
                }
        ...
        case R_CIRCLE_TRACK:
        {
            for(int i = -10;i <= 10;i++)
            {
                if((Img_Store_p -> Img_OTSU_Unpivot).at<uchar>((Data_Path_p -> InterruptCoordinate[0][3]-10),(Data_Path_p -> InterruptCoordinate[0][2])+i) == 255)
                {
                    CircleStepFlag = 1;
                    break;
                }
    ...
    {
        case 0:{ CircleTrack_Step = IN_PREPARE; break; }
...
void CircleTrack_Step_IN_Prepare(Img_Store *Img_Store_p,Data_Path *Data_Path_p)
{
    switch(Data_Path_p -> Track_Kind)
    {
        case L_CIRCLE_TRACK:
        {
            // 准备左入环补线
            // 赛道彩色图像
            line((Img_Store_p -> Img_Track_Unpivot),Point((Data_Path_p -> InterruptCoordinate[0][0]),(Data_Path_p -> InterruptCoordinate[0][1])),Point((Data_Path_p -> SideCoordinate_Eight[(Data_Path_p -> NumSearch[0])-1][0]),(Data_Path_p -> SideCoordinate_Eight[(Data_Path_p -> NumSearch[0])-1][1])),Scalar(128,0,128),4);
            // 赛道二值化图像
            line((Img_Store_p -> Img_OTSU_Unpivot),Point((Data_Path_p -> InterruptCoordinate[0][0]),(Data_Path_p -> InterruptCoordinate[0][1])),Point((Data_Path_p -> SideCoordinate_Eight[(Data_Path_p -> NumSearch[0])-1][0]),(Data_Path_p -> SideCoordinate_Eight[(Data_Path_p -> NumSearch[0])-1][1])),Scalar(255),4);
        ...
        case R_CIRCLE_TRACK:
        {
            // 准备右入环补线
            // 赛道彩色图像
            line((Img_Store_p -> Img_Track_Unpivot),Point((Data_Path_p -> InterruptCoordinate[0][2]),(Data_Path_p -> InterruptCoordinate[0][3])),Point((Data_Path_p -> SideCoordinate_Eight[(Data_Path_p -> NumSearch[0])-1][2]),(Data_Path_p -> SideCoordinate_Eight[(Data_Path_p -> NumSearch[0])-1][3])),Scalar(128,0,128),4);
            // 赛道二值化图像
            line((Img_Store_p -> Img_OTSU_Unpivot),Point((Data_Path_p -> InterruptCoordinate[0][2]),(Data_Path_p -> InterruptCoordinate[0][3])),Point((Data_Path_p -> SideCoordinate_Eight[(Data_Path_p -> NumSearch[0])-1][2]),(Data_Path_p -> SideCoordinate_Eight[(Data_Path_p -> NumSearch[0])-1][3])),Scalar(255),4);
